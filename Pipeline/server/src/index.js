...
// app.get("/search", async (req, res) => {
//   try {
//     const { q, topK, documentId, engine } = req.query;
//     if (!q) return res.status(400).json({ error: "Missing ?q=" });

//     let results;
//     if (engine === "pinecone") {
//       // 🔹 Search in Pinecone
//       results = await semanticSearchPinecone({
//         query: q,
//         topK: topK ? Number(topK) : 5,
//       });
//     } else {
//       // 🔹 Default: Search in Supabase pgvector
...
app.get("/document/:id/text", async (req, res) => {
  try {
    ...
    res.json({ document_id: id, text: fullText });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
